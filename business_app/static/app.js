const APS=window.APS_CONSTANTS;
const state={token:localStorage.getItem('aps_token')||'',user:null,systemConfig:null,page:'dashboard',masterType:'order',masterOrderType:APS.scheduleType.MACHINING,masterCalendarType:APS.scheduleType.MACHINING,masterRecords:[],versions:[],selectedVersionIds:new Set(),users:[],effectiveData:null,effectiveFilters:{},effectiveView:'order',lockTarget:null,dragAdjustment:null,pendingAdjustment:null,criticalPathAnalysis:null,criticalPathItems:new Map(),criticalPathsByProcessId:new Map(),selectedCriticalPathId:'',planVersionView:null};
const $=s=>document.querySelector(s); const content=$('#content');
const labels={dashboard:['OPERATIONS CENTER','生产运营总览'],tasks:['SCHEDULING JOBS','排程任务中心'],versions:['PLAN GOVERNANCE','计划版本管理'],effective:['EFFECTIVE SCHEDULE','生效排程看板'],master:['MASTER DATA','主数据中心'],settings:['SYSTEM PROFILE','业务单元配置'],users:['ACCESS CONTROL','用户与权限'],audit:['AUDIT TRAIL','系统审计日志']};
...
const processStatusLabels=APS.processStatusLabels;
...
function userFacingMessage(msg){return String(msg??'').replaceAll('TOPSIS 目标权重','方案选择权重').replaceAll('TOPSIS 权重','方案选择权重')}
function toast(msg,error=false){const el=$('#toast');el.textContent=userFacingMessage(msg);el.className='toast'+(error?' error':'');setTimeout(()=>el.classList.add('hidden'),3000)}
...
function showModal(title,html){document.querySelector('.modal-card').classList.remove('plan-modal','task-modal','task-detail-modal','compare-modal','multi-compare-modal','schedule-task-modal');$('#modalTitle').textContent=title;$('#modalBody').innerHTML=html;$('#modal').classList.remove('hidden')}
function closeModal(){$('#modal').classList.add('hidden');state.planVersionView=null;if(state.page==='effective'&&state.effectiveData)setCriticalPathContext(state.effectiveData.critical_path_analysis||null,state.effectiveData.schedule||[]);else setCriticalPathContext(null,[])}
...
async function startApp(){try{if(!state.user)state.user=await api('/api/auth/me');state.systemConfig=await api('/api/system-configuration');applySystemConfiguration();$('#loginView').classList.add('hidden');$('#appView').classList.remove('hidden');$('#userBadge').innerHTML=`<strong>${esc(state.user.display_name)}</strong><small>${esc(state.user.role)}</small>`;navigate(state.page)}catch{logout()}}
function navigate(page){setCriticalPathContext(null);state.page=page;document.querySelectorAll('#nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===page));$('#pageEyebrow').textContent=labels[page][0];$('#pageTitle').textContent=labels[page][1];content.innerHTML='<div class="loading">正在加载…</div>';({dashboard:renderDashboard,tasks:renderTasks,versions:renderVersions,effective:renderEffectiveSchedule,master:renderMaster,settings:renderSystemConfiguration,users:renderUsers,audit:renderAudit}[page])()}
document.querySelectorAll('#nav button').forEach(b=>b.onclick=()=>navigate(b.dataset.page));
const deploymentLabels=APS.deploymentLabels;
const deploymentScheduleTypes=APS.deploymentScheduleTypes;
function applySystemConfiguration(){
    const config=state.systemConfig||{};
    $('#systemBrandTitle').textContent=config.system_display_name||'智能排程';
    document.title=(config.system_display_name||'智能排程')+' · APS';
    const fixed=deploymentScheduleTypes[config.deployment_process_type];
    if(fixed)state.masterOrderType=fixed;
}
async function renderSystemConfiguration(){
    try{
        const config=await api('/api/system-configuration');
        state.systemConfig=config;applySystemConfiguration();
        const canEdit=state.user?.role==='admin';
        content.innerHTML=`<section class="panel system-config-panel"><div class="panel-header"><div><h3>业务系统部署配置</h3><p>部署工艺决定订单范围、任务工艺和系统标题；正式环境建议锁定。</p></div>${config.deployment_locked?badge('LOCKED'):badge('EDITABLE')}</div><form id="systemConfigForm" class="form-grid"><label>部署工艺类型<select name="deployment_process_type" ${canEdit?'':'disabled'}>${['MACHINING','HEAT_TREATMENT','ASSEMBLY','DEBUG'].map(value=>`<option value="${value}" ${config.deployment_process_type===value?'selected':''}>${deploymentLabels[value]}</option>`).join('')}</select></label><label>系统显示名称<input name="system_display_name" maxlength="40" value="${esc(config.system_display_name||'')}" ${canEdit?'':'readonly'} required></label><label class="full"><span><input name="deployment_locked" type="checkbox" ${config.deployment_locked?'checked':''} ${canEdit?'':'disabled'}> 部署后锁定配置</span></label><label class="full">修改原因<textarea name="change_reason" rows="3" ${canEdit?'':'readonly'} placeholder="说明本次修改原因"></textarea></label>${canEdit?`<label class="full config-confirmations"><span><input name="confirm_unlock" type="checkbox"> 当前配置已锁定时，确认解锁并修改</span><span><input name="confirm_published_versions" type="checkbox"> 存在已发布计划时，确认仍要切换业务单元</span></label>`:''}<div class="system-config-meta full"><span>配置修改人：<b>${esc(config.updated_by||'-')}</b></span><span>配置修改时间：<b>${esc(config.updated_at||'-')}</b></span><span>允许任务工艺：<b>${esc((config.allowed_schedule_types||[]).map(scheduleTypeLabel).join('、'))}</b></span></div>${canEdit?'<div class="form-actions"><button class="button primary" type="submit">保存部署配置</button></div>':''}</form></section>`;
        if(canEdit)$('#systemConfigForm').onsubmit=async event=>{event.preventDefault();const form=event.target,payload=Object.fromEntries(new FormData(form));payload.deployment_locked=form.deployment_locked.checked;payload.confirm_unlock=form.confirm_unlock.checked;payload.confirm_published_versions=form.confirm_published_versions.checked;try{state.systemConfig=await api('/api/system-configuration',{method:'PUT',body:JSON.stringify(payload)});applySystemConfiguration();toast('部署配置已保存');renderSystemConfiguration()}catch(error){toast(error.message,true)}};
    }catch(error){content.innerHTML=errorHtml(error)}
}
const openTaskModalBase=openTaskModal;
openTaskModal=async function(){
    const deployment=state.systemConfig?.deployment_process_type||APS.deployment.DEBUG;
    const fixedType=deploymentScheduleTypes[deployment]||'';
    let defaults={process_count:0,population_size:56,generations:30,schedule_type:fixedType};
    if(fixedType){try{defaults=await api(`/api/tasks/defaults?schedule_type=${encodeURIComponent(fixedType)}`);state.defaultScheduleStart=defaults.schedule_start||''}catch{state.defaultScheduleStart=''}}else{state.defaultScheduleStart=''}
    const result=openTaskModalBase();
    const select=$('#taskForm [name="schedule_type"]');
    if(deployment===APS.deployment.DEBUG){
        select.insertAdjacentHTML('afterbegin','<option value="" selected>请选择工艺类型</option>');
        select.required=true;
    }else{
        select.value=fixedType;
        select.disabled=true;
        select.insertAdjacentHTML('afterend',`<input type="hidden" name="schedule_type" value="${esc(fixedType)}">`);
    }
    enhanceTaskModal(defaults);
    return result;
};
...
async function renderTasks(){try{const rows=await api('/api/tasks');content.innerHTML=`<section class="panel"><div class="panel-header"><div><h3>排程任务队列</h3><p>静态全量、动态滚动和局部微调统一管理</p></div><button class="button primary" onclick="openTaskModal()">＋ 新建任务</button></div>${taskTable(rows)}</section>`;if(rows.some(r=>APS.activeTaskStatuses.includes(r.status)))setTimeout(()=>state.page==='tasks'&&renderTasks(),3000)}catch(e){content.innerHTML=errorHtml(e)}}
...
function openTaskModal(){showModal('新建排程任务',`<form id="taskForm" class="form-grid"><label>工艺类型<select name="schedule_type"><option value="machining">机加</option><option value="heat_treatment">热表</option><option value="assembly">装配</option></select></label><label>排程模式<select name="mode" id="taskMode"><option value="static">静态全量排程</option><option value="dynamic">动态滚动排程</option><option value="local">局部微调</option></select></label><label>派工规则<select name="dispatching_rule"><option value="DELIVERY">交期优先(EDD)</option><option value="PRIORITY">优先级优先(PRIORITY)</option><option value="SLACK">最小松弛时间(SLACK)</option><option value="EFFICIENCY">效率优先(EFFICIENCY)</option><option value="FCFS">先到先服务(FCFS)</option></select></label><label>排程基准时间<input name="schedule_start" type="datetime-local" value="${localDateTimeValue()}" required></label><label id="protectionField" style="display:none">滚动保护窗口（小时）<input name="protection_hours" type="number" min="0" step="1" value="24"><span class="field-help">保护计划开工前后窗口内的 CONFIRMED 工序</span></label><div class="full task-config-section"><h4>方案选择权重</h4><div class="weight-grid"><label>最大完工时间<input name="weight_makespan" type="number" min="0" step="0.01" value="0.15"></label><label>延期订单数<input name="weight_tardiness_count" type="number" min="0" step="0.01" value="0.20"></label><label>总延期<input name="weight_total_tardiness" type="number" min="0" step="0.01" value="0.15"></label><label>设备空闲率<input name="weight_machine_idle_rate" type="number" min="0" step="0.01" value="0.20"></label><label>设备负荷均衡<input name="weight_machine_balance" type="number" min="0" step="0.01" value="0.15"></label><label>工人负荷均衡<input name="weight_worker_balance" type="number" min="0" step="0.01" value="0.05"></label><label>WIP 等待<input name="weight_wip_waiting" type="number" min="0" step="0.01" value="0.10"></label></div><p id="weightTotal" class="weight-total">权重合计：1.00</p></div><label class="full">其他算法参数覆盖（JSON）<textarea name="config_overrides" rows="6">{"nsga3":{"population_size":8,"generations":3},"tabu_search":{"enabled":false}}</textarea></label><label class="full" id="localField" style="display:none">局部调整（JSON 数组）<textarea name="local_adjustments" rows="7">[]</textarea></label><div class="form-actions"><button type="button" class="button ghost" data-close-modal>取消</button><button class="button primary" type="submit">提交算法任务</button></div></form>`);document.querySelector('.modal-card').classList.add('task-modal');const syncModeFields=()=>{const mode=$('#taskMode').value;$('#localField').style.display=mode==='local'?'flex':'none';$('#protectionField').style.display=mode==='dynamic'?'flex':'none'};const weightNames=['weight_makespan','weight_tardiness_count','weight_total_tardiness','weight_machine_idle_rate','weight_machine_balance','weight_worker_balance','weight_wip_waiting'];const updateWeightTotal=()=>{const total=weightNames.reduce((sum,name)=>sum+(Number(document.querySelector(`[name="${name}"]`).value)||0),0);$('#weightTotal').textContent=`权重合计：${total.toFixed(2)}`;$('#weightTotal').classList.toggle('invalid',Math.abs(total-1)>0.0001)};$('#taskMode').onchange=syncModeFields;weightNames.forEach(name=>document.querySelector(`[name="${name}"]`).addEventListener('input',updateWeightTotal));syncModeFields();$('#taskForm').onsubmit=async e=>{e.preventDefault();const fd=Object.fromEntries(new FormData(e.target));try{const config=JSON.parse(fd.config_overrides||'{}'),weights={};for(const name of weightNames){const value=Number(fd[name]);if(!Number.isFinite(value)||value<0)throw new Error('方案选择权重必须是大于等于 0 的数字');weights[name.replace('weight_','')]=value;delete fd[name]}const total=Object.values(weights).reduce((sum,value)=>sum+value,0);if(Math.abs(total-1)>0.0001)throw new Error(`方案选择权重合计必须为 1，当前为 ${total.toFixed(2)}`);config.topsis={...(config.topsis||{}),weights};const protectionHours=Number(fd.protection_hours);delete fd.protection_hours;if(fd.mode==='dynamic'){if(!Number.isFinite(protectionHours)||protectionHours<0)throw new Error('滚动保护窗口必须是大于等于 0 的数字');config.rolling_window={...(config.rolling_window||{}),confirmed_protection_hours:protectionHours}}fd.config_overrides=config;fd.local_adjustments=JSON.parse(fd.local_adjustments||'[]');const r=await api('/api/tasks',{method:'POST',body:JSON.stringify(fd)});closeModal();toast(`任务 ${r.task_id} 已进入队列`);navigate('tasks')}catch(err){toast(err.message,true)}}}
function taskPayloadSize(value){const bytes=Number(value)||0;if(bytes<1024)return `${bytes} B`;if(bytes<1024*1024)return `${roundMetric(bytes/1024)} KB`;return `${roundMetric(bytes/1024/1024)} MB`}
function taskSnapshotSummary(summary={},sizes={}){const items=[['订单',summary.order_count||0],['工序',summary.process_count||0],['设备',summary.machine_count||0],['人员',summary.worker_count||0],['资源组',summary.resource_group_count||0],['工厂日历',summary.calendar_configured?'已配置':'未配置']];return `<div class="task-snapshot-head"><div><strong>任务快照摘要</strong><span>完整快照 ${esc(taskPayloadSize(sizes.snapshot))}，不在弹框中展开</span></div></div><div class="task-snapshot-grid">${items.map(([label,value])=>`<div><small>${esc(label)}</small><strong>${esc(value)}</strong></div>`).join('')}</div>`}
function taskPayloadButtons(id,kinds,sizes={}){const labels={request:'完整请求',snapshot:'完整快照',response:'完整结果'};return `<div class="task-payload-actions">${kinds.map(kind=>`<button type="button" class="button ghost small" onclick="downloadTaskPayload('${esc(id)}','${kind}')">下载${labels[kind]} · ${esc(taskPayloadSize(sizes[kind]))}</button>`).join('')}</div>`}
async function downloadTaskPayload(id,kind){try{const headers={};if(state.token)headers.Authorization=`Bearer ${state.token}`;const response=await fetch(`/api/tasks/${encodeURIComponent(id)}/payload/${encodeURIComponent(kind)}`,{headers});if(!response.ok){let data={};try{data=await response.json()}catch{}throw new Error(data.detail||`下载失败 ${response.status}`)}const blob=await response.blob(),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`${id}-${kind}.json`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)}catch(error){toast(error.message,true)}}
async function showTask(id){try{const t=await api(`/api/tasks/${encodeURIComponent(id)}/summary`),sizes=t.payload_sizes||{},responseData=t.response||{status:t.status,message:['QUEUED','RUNNING'].includes(t.status)?'任务尚未完成，暂无算法结果':'暂无结果数据'};showModal(`任务详情 · ${id}`,`<div class="detail-head"><div>${badge(t.status)}<p class="muted">${esc(t.schedule_type)} / ${esc(t.mode)} / ${esc(comparisonDispatchingLabel(t.dispatching_rule))}</p></div>${t.status==='FAILED'?'<button id="retryTaskButton" type="button" class="button danger task-retry-button">↻ 重试任务</button>':''}</div>${t.error_message?`<p class="form-error">${esc(t.error_message)}</p>`:''}<div class="task-data-tabs" role="tablist" aria-label="任务数据类型"><button class="task-data-tab active" type="button" role="tab" aria-selected="true" aria-controls="task-request-panel" data-task-data-tab="request" onclick="switchTaskData('request',this)">请求摘要</button><button class="task-data-tab" type="button" role="tab" aria-selected="false" aria-controls="task-result-panel" data-task-data-tab="result" onclick="switchTaskData('result',this)">结果摘要</button></div><section class="task-data-section task-data-panel" id="task-request-panel" role="tabpanel" data-task-data-panel="request"><div class="task-data-header"><p>仅展示任务参数摘要；完整大数据通过下载获取</p><span class="badge">REQUEST</span></div>${taskSnapshotSummary(t.snapshot_summary,sizes)}<pre class="code-block task-code-block task-summary-code">${esc(JSON.stringify(t.request||{},null,2))}</pre>${taskPayloadButtons(id,['request','snapshot'],sizes)}</section><section class="task-data-section task-data-panel hidden" id="task-result-panel" role="tabpanel" data-task-data-panel="result"><div class="task-data-header"><p>仅展示状态、KPI、优化器和结果数量摘要</p>${badge(responseData.status||t.status)}</div><pre class="code-block task-code-block task-summary-code">${esc(JSON.stringify(responseData,null,2))}</pre>${t.response_available?taskPayloadButtons(id,['response'],sizes):''}</section>`);document.querySelector('.modal-card').classList.add('task-detail-modal');const retryButton=$('#retryTaskButton');if(retryButton)retryButton.onclick=()=>retryTask(id,retryButton)}catch(e){toast(e.message,true)}}
...
async function retryTask(id,button=null){if(button){button.disabled=true;button.textContent='正在重试…'}try{await api(`/api/tasks/${encodeURIComponent(id)}/retry`,{method:'POST'});closeModal();toast('任务已重新进入队列');renderTasks()}catch(e){if(button){button.disabled=false;button.textContent='↻ 重试任务'}toast(e.message,true)}}
...
function scheduleTypeLabel(value){return(APS.scheduleTypeLabels[value]||value||'-')}
function virtualResourceTag(item){return item?.virtual_resource?'<span class="badge VIRTUAL">虚拟资源</span>':''}
...
const renderEffectiveScheduleBase=renderEffectiveSchedule;
renderEffectiveSchedule=async function(){setCriticalPathContext(null,[]);await renderEffectiveScheduleBase();setCriticalPathContext(state.effectiveData?.critical_path_analysis||null,state.effectiveData?.schedule||[])};
...
function effectiveScheduleTable(rows){if(!rows.length)return '<div class="empty">当前筛选条件下没有工序</div>';return `<div class="table-wrap effective-table-wrap"><table class="data-table effective-table"><thead><tr><th>工序</th><th>生效状态</th><th>工序状态</th><th>订单</th><th>设备</th><th>人员</th><th>计划开始</th><th>计划完工</th><th>合批号</th><th>来源版本</th><th>异常</th></tr></thead><tbody>${rows.map(row=>`<tr class="${row.has_conflict?'conflict-row':''}"><td><b>${esc(row.process_id)}</b><div class="record-sub">${esc(row.process_name||'')}</div></td><td>${badge(row.schedule_state)}</td><td><div class="status-lock-cell">${processStatusBadge(row.status||'-')}${lockMarker(row,true)}</div></td><td><b>${esc(row.order_id||'-')}</b><div class="record-sub">${esc(row.product_name||'')}</div></td><td>${virtualResourceTag(row)}${esc(row.machine_id||'-')}<div class="record-sub">${esc(row.machine_name||'')}</div></td><td>${esc(row.worker_id||'-')}<div class="record-sub">${esc(row.worker_name||'')}</div></td><td>${esc(row.plan_start_time||'-')}</td><td>${esc(row.plan_end_time||'-')}</td><td class="mono">${esc(row.batch_id||'-')}</td><td>${row.schedule_version_id?`<button class="button ghost small mono" onclick="showVersion('${esc(row.schedule_version_id)}')">${esc(row.schedule_version_id)}</button>`:'-'}</td><td>${row.has_conflict?`<span class="effective-conflict-tag">${esc(row.conflict_types.join(' / '))} 冲突</span>`:'-'}</td></tr>`).join('')}</tbody></table></div>`}
...
async function showVersion(id){
    try{
        ...
        setCriticalPathContext(result.critical_path_analysis||null,rows);
        ...
        const loadMachineCount=new Set(rows.filter(x=>x.machine_id&&!x.virtual_resource).map(x=>x.machine_id)).size;
        const workerCount=new Set(rows.map(x=>x.worker_id).filter(Boolean)).size;
        const actions=[];
        if(v.status===APS.versionStatus.DRAFT)actions.push(`<button class="button secondary" onclick="reviewVersion('${esc(id)}','${APS.versionStatus.REJECTED}')">驳回</button><button class="button primary" onclick="reviewVersion('${esc(id)}','${APS.versionStatus.APPROVED}')">审批通过</button>`);
        if(v.status===APS.versionStatus.APPROVED)actions.push(`<button class="button primary" onclick="publishPlan('${esc(id)}')">发布计划</button>`);
        state.planVersionView={versionId:String(id),rows,machineLoadContext:v.machine_load_context||{},rendered:{},rendering:{},orderGroups:{},detailPage:1,detailPageSize:50};
        showModal(`计划版本 · ${id}`,`<div class="detail-head"><div>${badge(v.status)}<div class="version-title-line"><h3>${esc(id)}</h3>${versionScoreBadge(result)}</div><p class="muted">${esc(v.schedule_type)} · 来源 ${esc(v.task_id)}</p></div><div class="actions">${actions.join('')}</div></div>${versionParameterPanel(v)}<div class="kpi-grid">${kpiCards(result.kpis||{},result.best_objectives||{},result.metadata||{})}</div>${criticalPathOverview(result.critical_path_analysis||null,rows)}<div class="gantt-switcher" role="tablist"><button class="gantt-switch active" role="tab" aria-selected="true" onclick="switchPlanGantt('order',this)">订单工序甘特图</button><button class="gantt-switch" role="tab" aria-selected="false" onclick="switchPlanGantt('machine',this)">设备甘特图</button><button class="gantt-switch" role="tab" aria-selected="false" onclick="switchPlanGantt('worker',this)">工人甘特图</button><button class="gantt-switch" role="tab" aria-selected="false" onclick="switchPlanGantt('detail',this)">排程明细</button><button class="gantt-switch" role="tab" aria-selected="false" onclick="switchPlanGantt('load',this)">设备负荷图</button></div><div class="gantt-view" data-gantt-view="order"><div class="gantt-view-note"><span>先显示订单汇总；点击订单时再加载内部工序和关键路径标记</span><span class="badge">${orderCount} 个订单</span></div><div data-plan-view-content="order"><div class="plan-view-loading">正在准备订单汇总…</div></div></div><div class="gantt-view hidden" data-gantt-view="machine"><div class="gantt-view-note"><span>首次切换到本页签时加载设备甘特图</span><span class="badge">${machineCount} 台设备</span></div><div data-plan-view-content="machine"><div class="plan-view-placeholder">切换到设备页签后加载</div></div></div><div class="gantt-view hidden" data-gantt-view="worker"><div class="gantt-view-note"><span>首次切换到本页签时加载人员甘特图</span><span class="badge">${workerCount} 名工人</span></div><div data-plan-view-content="worker"><div class="plan-view-placeholder">切换到人员页签后加载</div></div></div><div class="gantt-view hidden" data-gantt-view="detail"><div class="gantt-view-note"><span>排程明细按每页 50 道工序加载</span><span class="badge">${rows.length} 道工序</span></div><div data-plan-view-content="detail"><div class="plan-view-placeholder">切换到排程明细后加载</div></div></div><div class="gantt-view hidden" data-gantt-view="load"><div class="gantt-view-note"><span>按真实设备汇总计划占用工时；热表合批只计算一次设备占用。计算公式：设备负荷率 = 设备计划占用工时 ÷ 计划周期内设备可用工时 × 100%。</span><span class="badge">${loadMachineCount} 台真实设备</span></div><div data-plan-view-content="load"><div class="plan-view-placeholder">切换到设备负荷图后加载</div></div></div>`);
        document.querySelector('.modal-card').classList.add('plan-modal');
        requestAnimationFrame(()=>{if(state.planVersionView?.versionId===String(id))renderPlanGanttView('order')});
    }catch(e){toast(e.message,true)}
}
function criticalPathOverview(analysis,rows){if(!analysis?.enabled)return '<section class="critical-overview disabled compact"><div class="critical-overview-metrics"><span>关键路径分析未启用</span></div></section>';const structural=analysis.structural||{},delivery=analysis.delivery||{},resource=analysis.resource||{},overdue=Number(delivery.overdue_process_count)||rows.filter(item=>item.delivery_critical_level==='OVERDUE').length;return `<section class="critical-overview compact"><div class="critical-overview-metrics"><span class="structural"><b>${Number(structural.critical_process_count)||0}</b> 结构关键</span><span class="delivery ${overdue?'danger':''}"><b>${Number(delivery.critical_process_count)||0}</b> 交付关键</span><span class="danger"><b>${overdue}</b> 已延期</span><span class="resource"><b>${Number(resource.critical_process_count)||0}</b> 资源关键</span></div></section>`}
function switchPlanGantt(kind,button){const modal=button.closest('.modal-card');modal.querySelectorAll('.gantt-switch').forEach(item=>{const active=item===button;item.classList.toggle('active',active);item.setAttribute('aria-selected',String(active))});modal.querySelectorAll('.gantt-view').forEach(view=>view.classList.toggle('hidden',view.dataset.ganttView!==kind));renderPlanGanttView(kind)}
function renderPlanGanttView(kind){const context=state.planVersionView,container=document.querySelector(`[data-plan-view-content="${kind}"]`);if(!context||!container||context.rendered[kind]||context.rendering[kind])return;context.rendering[kind]=true;container.innerHTML='<div class="plan-view-loading">正在加载视图…</div>';const versionId=context.versionId;requestAnimationFrame(()=>{if(state.planVersionView?.versionId!==versionId||!container.isConnected)return;if(kind==='detail'){container.innerHTML='<div id="planScheduleTable"><div class="plan-view-loading">正在加载排程明细…</div></div>';context.rendered[kind]=true;context.rendering[kind]=false;renderPlanSchedulePage(context.detailPage);return}if(kind==='load'){container.innerHTML=renderMachineLoadChart(context.rows,context.machineLoadContext);context.rendered[kind]=true;context.rendering[kind]=false;return}container.innerHTML=kind==='order'?renderPlanOrderGantt(context.rows):kind==='machine'?renderResourceGantt(context.rows,'machine_id','未分配设备','machine'):renderResourceGantt(context.rows,'worker_id','未分配人员','worker');context.rendered[kind]=true;context.rendering[kind]=false;applyCriticalPathHighlight()})}
function planTaskLoadMinutes(item){const direct=Number(item?.total_duration_minutes);if(Number.isFinite(direct)&&direct>=0)return direct;const start=new Date(item?.plan_start_time).getTime(),end=new Date(item?.plan_end_time).getTime();return Number.isFinite(start)&&Number.isFinite(end)&&end>start?(end-start)/60000:0}
function loadMetricPercent(value){return Number.isFinite(value)?`${roundMetric(value)}%`:'-'}
function loadPeriodTime(value){return value?String(value).replace('T',' ').slice(0,16):'-'}
function renderMachineLoadChart(rows,context={}){
    const calendarByMachine=new Map((context.machines||[]).map(item=>[String(item.machine_id),item]));
    const groups=new Map();
    for(const item of rows||[]){
        const machineId=String(item?.machine_id||'');
        if(!machineId||item?.virtual_resource)continue;
        const calendarInfo=calendarByMachine.get(machineId)||{};
        let group=groups.get(machineId);
        if(!group){
            group={machine_id:machineId,machine_name:item.machine_name||calendarInfo.machine_name||'',available_minutes:Number(calendarInfo.available_minutes)||0,process_ids:new Set(),occupancies:new Map()};
            groups.set(machineId,group);
        }else if(!group.machine_name&&(item.machine_name||calendarInfo.machine_name)){
            group.machine_name=item.machine_name||calendarInfo.machine_name;
        }
        if(item.process_id)group.process_ids.add(String(item.process_id));
        const occupancyKey=item.batch_id?`batch:${item.batch_id}`:`process:${item.process_id||group.process_ids.size}`;
        const minutes=planTaskLoadMinutes(item);
        group.occupancies.set(occupancyKey,Math.max(group.occupancies.get(occupancyKey)||0,minutes));
    }
    const loads=[...groups.values()].map(group=>({...group,load_minutes:[...group.occupancies.values()].reduce((sum,value)=>sum+value,0)})).sort((a,b)=>b.load_minutes-a.load_minutes||a.machine_id.localeCompare(b.machine_id));
    if(!loads.length)return '<div class="empty">当前计划没有已分配的真实设备任务</div>';
    const totalMinutes=loads.reduce((sum,item)=>sum+item.load_minutes,0);
    const totalAvailableMinutes=loads.reduce((sum,item)=>sum+item.available_minutes,0);
    const maxMinutes=Math.max(...loads.map(item=>item.load_minutes),0);
    const averageMinutes=loads.length?totalMinutes/loads.length:0;
    const overallCalendarRate=totalAvailableMinutes?totalMinutes/totalAvailableMinutes*100:null;
    const period=`${loadPeriodTime(context.period_start)} ～ ${loadPeriodTime(context.period_end)}`;
    const calendarNote=context.calendar_configured?'所有设备使用相同计划周期，并按各自日历计算可用工时':'当前任务快照未配置有效设备日历，日历负荷率无法计算';
    return `<div class="machine-load-period ${context.calendar_configured?'':'warning'}"><span>计划时间周期</span><strong>${esc(period)}</strong><small>${esc(calendarNote)}</small></div><div class="machine-load-summary"><div><small>设备数量</small><strong>${loads.length}</strong></div><div><small>设备总负荷</small><strong>${roundMetric(totalMinutes/60)} h</strong></div><div><small>设备总可用工时</small><strong>${roundMetric(totalAvailableMinutes/60)} h</strong></div><div><small>总体日历负荷率</small><strong>${esc(loadMetricPercent(overallCalendarRate))}</strong></div><div><small>平均设备负荷</small><strong>${roundMetric(averageMinutes/60)} h</strong></div><div><small>最高设备负荷</small><strong>${roundMetric(maxMinutes/60)} h</strong></div></div><div class="machine-load-list">${loads.map(item=>{const share=totalMinutes?item.load_minutes/totalMinutes*100:0,calendarRate=item.available_minutes?item.load_minutes/item.available_minutes*100:null,width=calendarRate===null?0:Math.max(Math.min(calendarRate,100),calendarRate?1:0),rateClass=calendarRate>100?'danger':calendarRate>=85?'warning':'';return `<div class="machine-load-row ${rateClass}"><div class="machine-load-label"><strong>${esc(item.machine_id)}</strong><small>${esc(item.machine_name||'未记录设备名称')} · ${item.process_ids.size} 道工序</small></div><div class="machine-load-track" title="日历负荷率 ${esc(loadMetricPercent(calendarRate))}"><i style="width:${width}%"></i></div><div class="machine-load-value"><strong>${roundMetric(item.load_minutes/60)} h / 可用 ${roundMetric(item.available_minutes/60)} h</strong><div class="machine-load-rates"><span>负荷占比 <b>${esc(loadMetricPercent(share))}</b></span><span class="${rateClass}">日历负荷率 <b>${esc(loadMetricPercent(calendarRate))}</b></span></div></div></div>`}).join('')}</div>`;
}
...
function openManualLockModal(){
    hideProcessContextMenu();const item=state.lockTarget;if(!item)return;
    const locks=item.locks||{},options=item.lock_options||{},startValue=locks.start_time||item.plan_start_time||'',endValue=locks.end_time||item.plan_end_time||'';
    const resourceFields=item.virtual_resource?'':`<label>\u9501\u5b9a\u8bbe\u5907<select name="machine_id"><option value="">\u4e0d\u9501\u5b9a\u8bbe\u5907</option>${(options.machines||[]).map(x=>selectedOption(x.machine_id,locks.machine_id||item.machine_id||'',`${x.machine_id} \u00b7 ${x.machine_name||''}`)).join('')}</select></label><label>\u9501\u5b9a\u4eba\u5458<select name="worker_id"><option value="">\u4e0d\u9501\u5b9a\u4eba\u5458</option>${(options.workers||[]).map(x=>selectedOption(x.worker_id,locks.worker_id||item.worker_id||'',`${x.worker_id} \u00b7 ${x.worker_name||''}`)).join('')}</select></label>`;
    const help=item.virtual_resource?'\u865a\u62df\u5de5\u5e8f\u53ea\u5141\u8bb8\u9501\u5b9a\u8ba1\u5212\u65f6\u95f4\uff0c\u4e0d\u80fd\u6307\u5b9a\u8bbe\u5907\u6216\u4eba\u5458\u3002':'\u8bbe\u5907\u3001\u4eba\u5458\u548c\u65f6\u95f4\u53ef\u4ee5\u6309\u9700\u9501\u5b9a\uff1b\u5f00\u59cb\u4e0e\u7ed3\u675f\u65f6\u95f4\u5fc5\u987b\u540c\u65f6\u586b\u5199\u3002';
    showModal(`${item.manually_locked?'\u4fee\u6539':'\u65b0\u589e'}\u4eba\u5de5\u9501 \u00b7 ${item.process_id}`,`<form id="manualLockForm" class="form-grid">${resourceFields}<label>\u9501\u5b9a\u5f00\u59cb\u65f6\u95f4<input name="start_time" type="datetime-local" value="${esc(lockDateTimeValue(startValue))}" required></label><label>\u9501\u5b9a\u7ed3\u675f\u65f6\u95f4<input name="end_time" type="datetime-local" value="${esc(lockDateTimeValue(endValue))}" required></label><label class="full">\u9501\u5b9a\u539f\u56e0<textarea name="lock_reason" rows="4" maxlength="500" required>${esc(locks.lock_reason||'')}</textarea></label><div class="full field-help">${help}</div><div class="form-actions"><button type="button" class="button ghost" data-close-modal>\u53d6\u6d88</button><button class="button primary" type="submit">\u4fdd\u5b58\u4eba\u5de5\u9501</button></div></form>`);
    document.querySelector('.modal-card').classList.add('task-modal');
    $('#manualLockForm').onsubmit=async event=>{event.preventDefault();const fd=Object.fromEntries(new FormData(event.target));try{await api(`/api/processes/${encodeURIComponent(item.process_id)}/lock`,{method:'POST',body:JSON.stringify({...fd,schedule_version_id:item.schedule_version_id,expected_lock_time:locks.lock_time||''})});closeModal();toast(`\u5de5\u5e8f ${item.process_id} \u5df2\u4eba\u5de5\u9501\u5b9a`);renderEffectiveSchedule()}catch(error){toast(error.message,true)}};
}
...
function openProcessAdjustmentModal(){
    hideProcessContextMenu();const item=state.lockTarget;if(!item||!item.allow_manual_adjustment)return;
    const options=item.lock_options||{machines:[],workers:[]};
    const resourceFields=item.virtual_resource?'':`<label>\u76ee\u6807\u8bbe\u5907<select name="assigned_machine_id" id="adjustmentMachine">${(options.machines||[]).map(x=>selectedOption(x.machine_id,item.machine_id,`${x.machine_id} \u00b7 ${x.machine_name||''}`)).join('')}</select></label><label>\u76ee\u6807\u4eba\u5458<select name="assigned_worker_id" id="adjustmentWorker">${(options.workers||[]).map(x=>selectedOption(x.worker_id,item.worker_id,`${x.worker_id} \u00b7 ${x.worker_name||''}`)).join('')}</select></label>`;
    const help=item.virtual_resource?'\u865a\u62df\u5de5\u5e8f\u53ea\u8c03\u6574\u9884\u8ba1\u8f6c\u5de5\u65f6\u95f4\uff0c\u4e0d\u91cd\u65b0\u5206\u914d\u8bbe\u5907\u548c\u4eba\u5458\u3002':'\u66f4\u6362\u8bbe\u5907\u4ec5\u5141\u8bb8\u9009\u62e9\u5f53\u524d\u8d44\u6e90\u7ec4\u5185\u8bbe\u5907\uff0c\u7cfb\u7edf\u4f1a\u7ee7\u7eed\u6821\u9a8c\u4eba\u5458\u6388\u6743\u3001\u65e5\u5386\u3001\u524d\u540e\u5e8f\u548c\u8d44\u6e90\u5360\u7528\u3002';
    showModal(`\u8c03\u6574\u5de5\u5e8f \u00b7 ${item.process_id}`,`<form id="processAdjustmentForm" class="form-grid"><div class="full adjustment-summary"><strong>${esc(item.process_id)} \u00b7 ${esc(item.process_name||'')}</strong><span>\u8ba2\u5355 ${esc(item.order_id||'-')} \u00b7 ${item.virtual_resource?'\u865a\u62df\u65e0\u9650\u8d44\u6e90':`\u5f53\u524d\u8bbe\u5907 ${esc(item.machine_id||'-')} \u00b7 \u5f53\u524d\u4eba\u5458 ${esc(item.worker_id||'-')}`}</span></div>${resourceFields}<label>\u65b0\u5f00\u5de5\u65f6\u95f4<input name="plan_start_time" type="datetime-local" value="${esc(adjustmentDateTimeValue(item.plan_start_time))}" required></label><label>\u65b0\u5b8c\u5de5\u65f6\u95f4<input name="plan_end_time" type="datetime-local" value="${esc(adjustmentDateTimeValue(item.plan_end_time))}" required></label><div class="full field-help">${help}</div><div class="form-actions"><button type="button" class="button ghost" data-close-modal>\u53d6\u6d88</button><button class="button primary" type="submit">\u6821\u9a8c\u5f71\u54cd</button></div></form>`);
    document.querySelector('.modal-card').classList.add('task-modal');
    if(!item.virtual_resource){const machine=$('#adjustmentMachine'),worker=$('#adjustmentWorker');machine.onchange=()=>{const selected=(options.machines||[]).find(x=>x.machine_id===machine.value),allowed=new Set(selected?.allowed_worker_ids||[]);[...worker.options].forEach(option=>option.disabled=allowed.size&&!allowed.has(option.value));if(worker.selectedOptions[0]?.disabled){const first=[...worker.options].find(option=>!option.disabled);if(first)worker.value=first.value}};machine.onchange()}
    $('#processAdjustmentForm').onsubmit=event=>{event.preventDefault();previewProcessAdjustment(item,Object.fromEntries(new FormData(event.target)))};
}
...
const openAdjustmentConfirmationBase=openAdjustmentConfirmation;
openAdjustmentConfirmation=function(){openAdjustmentConfirmationBase();const preview=state.pendingAdjustment?.preview;if(!preview?.critical_override_required)return;const reason=$('#adjustmentLockReason');if(!reason)return;const labels=(preview.critical_types||[]).map(type=>type==='delivery'?'交付关键':'资源关键').join('、');reason.insertAdjacentHTML('beforebegin',`<label class="adjustment-critical-authorization"><input type="checkbox" name="critical_override_authorized" required> 我已确认这是${esc(labels||'受保护关键')}工序，并授权覆盖关键路径保护</label>`)};
...
function scheduleTable(rows){return `<div class="table-wrap"><table class="data-table"><thead><tr><th>工序</th><th>状态</th><th>订单</th><th>设备</th><th>人员</th><th>开始</th><th>结束</th></tr></thead><tbody>${rows.map(x=>{const displayStatus=x.effective_status||x.status||x.source_status||'-';return `<tr><td><b>${esc(x.process_id)}</b><div class="record-sub">${esc(x.process_name||'')}</div></td><td><div class="status-lock-cell">${processStatusBadge(displayStatus)}${lockMarker(x,true)}</div>${x.effective_status&&x.status&&x.effective_status!==x.status?`<div class="record-sub">算法状态 ${esc(processStatusLabel(x.status))}</div>`:''}${x.source_status&&x.source_status!==displayStatus?`<div class="record-sub">排程前状态 ${esc(processStatusLabel(x.source_status))}</div>`:''}</td><td>${esc(x.order_id||'')}</td><td>${virtualResourceTag(x)}${esc(x.machine_id||'-')}</td><td>${esc(x.worker_id||'-')}</td><td>${esc(x.plan_start_time)}</td><td>${esc(x.plan_end_time)}</td></tr>`}).join('')}</tbody></table></div>`}
function renderPlanSchedulePage(page){const context=state.planVersionView,target=$('#planScheduleTable');if(!context||!target)return;const totalPages=Math.max(1,Math.ceil(context.rows.length/context.detailPageSize)),current=Math.max(1,Math.min(Number(page)||1,totalPages)),start=(current-1)*context.detailPageSize;context.detailPage=current;target.innerHTML=`${scheduleTable(context.rows.slice(start,start+context.detailPageSize))}<div class="plan-table-pagination"><span>第 ${current} / ${totalPages} 页 · 当前 ${Math.min(context.detailPageSize,Math.max(context.rows.length-start,0))} 道 · 共 ${context.rows.length} 道</span><div><button type="button" class="button ghost small" onclick="renderPlanSchedulePage(${current-1})" ${current<=1?'disabled':''}>上一页</button><button type="button" class="button ghost small" onclick="renderPlanSchedulePage(${current+1})" ${current>=totalPages?'disabled':''}>下一页</button></div></div>`}
...
function ganttTooltipFields(item){const materialTime=item.material_ready_time||'-',fields=[['订单号',item.order_id||'-'],['工序号',item.process_id||'-'],['工序状态',processStatusLabel(item.status||item.effective_status||'-')],['分配设备',item.machine_id||'-'],['分配工人',item.worker_id||'-'],['物料齐套时间',materialTime],['计划开始时间',item.plan_start_time||'-'],['计划完工时间',item.plan_end_time||'-']];if(item.virtual_resource)fields.push(['资源模式','虚拟无限资源']);if(item.batch_id)fields.push(['合批号',item.batch_id]);if(item.schedule_version_id||item.effective_schedule_version_id)fields.push(['生效版本',item.schedule_version_id||item.effective_schedule_version_id]);if(item.published_at)fields.push(['发布时间',item.published_at]);if(item.has_conflict)fields.push(['资源冲突',(item.conflict_types||[]).join(' / ')]);return fields}
function setCriticalPathContext(analysis,rows=[]){state.criticalPathAnalysis=analysis&&analysis.enabled?analysis:null;state.criticalPathItems=new Map((rows||[]).filter(item=>item?.process_id).map(item=>[String(item.process_id),item]));state.criticalPathsByProcessId=new Map();for(const path of state.criticalPathAnalysis?.all_paths||[])for(const processId of path.process_ids||[]){const key=String(processId),items=state.criticalPathsByProcessId.get(key)||[];items.push(path);state.criticalPathsByProcessId.set(key,items)}state.selectedCriticalPathId='';document.querySelectorAll('.gantt-path-selected,.gantt-path-muted').forEach(element=>element.classList.remove('gantt-path-selected','gantt-path-muted'));closeCriticalPathInfo()}
function criticalPathIds(item){return [...new Set(['delivery_path_ids','resource_path_ids'].flatMap(field=>Array.isArray(item?.[field])?item[field].map(String):[]))]}
function criticalProcessKey(item){return encodeURIComponent(String(item?.process_id||'')).replace(/'/g,'%27')}
function criticalTypeLabel(value){return value==='structural'?'结构路径':value==='delivery'?'交付路径':value==='resource'?'资源路径':value||'-'}
function deliveryCriticalLabel(value){return value==='OVERDUE'?'已延期关键':value==='CORE'?'核心交付关键':value==='NEAR'?'近交付关键':'普通'}
function criticalLabelDots(item){return `${item?.delivery_critical?`<i class="critical-label-dot delivery ${item.delivery_critical_level==='OVERDUE'?'negative':''}">D</i>`:''}${item?.resource_critical?'<i class="critical-label-dot resource">R</i>':''}`}
function criticalMarker(item){if(item?.is_order_summary||(!item?.delivery_critical&&!item?.resource_critical))return '';const key=criticalProcessKey(item),buttons=[];if(item.delivery_critical)buttons.push(`<button type="button" class="critical-marker delivery ${item.delivery_critical_level==='OVERDUE'?'negative':''}" onclick="event.stopPropagation();hideGanttTooltip();showCriticalPathInfo('${key}')" title="交付关键：按订单交期和时差计算">D</button>`);if(item.resource_critical)buttons.push(`<button type="button" class="critical-marker resource" onclick="event.stopPropagation();hideGanttTooltip();showCriticalPathInfo('${key}')" title="资源关键：设备和人员占用形成的实际路径">R</button>`);return `<span class="critical-markers">${buttons.join('')}</span>`}
function criticalMinutes(value){const number=Number(value);return Number.isFinite(number)?`${roundMetric(number,3)} 分钟`:'-'}
function criticalTime(value){return value?String(value).replace('T',' '):'-'}
function showCriticalPathInfo(encodedProcessId){let processId='';try{processId=decodeURIComponent(encodedProcessId)}catch{return}const item=state.criticalPathItems.get(String(processId));if(!item)return;const paths=(state.criticalPathsByProcessId.get(String(processId))||[]).filter(path=>path.critical_type!=='structural'),types=[item.delivery_critical?deliveryCriticalLabel(item.delivery_critical_level):null,item.resource_critical?'资源关键':null].filter(Boolean).join(' / ')||'普通工序',resourceReasons=(item.resource_critical_reasons||[]).map(reason=>reason==='makespan'?'影响总工期':reason==='delivery'?'影响订单交期':reason).join('、')||'-',details=[['工序',`${item.process_id||'-'} · ${item.process_name||''}`],['订单',item.order_id||'-'],['关键类型',types],['理论最早开始',criticalTime(item.theoretical_earliest_start_time)],['理论最早完成',criticalTime(item.theoretical_earliest_finish_time)],['交付总时差',criticalMinutes(item.delivery_total_slack_minutes)],['交付自由时差',criticalMinutes(item.delivery_free_slack_minutes)],['订单交付缓冲',criticalMinutes(item.order_delivery_slack_minutes)],['资源总工期时差',criticalMinutes(item.resource_total_slack_minutes)],['资源交付时差',criticalMinutes(item.resource_delivery_slack_minutes)],['资源关键原因',resourceReasons],['资源前置',Array.isArray(item.resource_predecessor_ids)&&item.resource_predecessor_ids.length?item.resource_predecessor_ids.join('、'):'-'],['资源约束',Array.isArray(item.resource_constraint_types)&&item.resource_constraint_types.length?item.resource_constraint_types.join('、'):'-'],['保护状态',item.delivery_critical||item.resource_critical?'局部调整需要明确授权':'仅展示，不强制保护']];const pathHtml=paths.length?`<div class="critical-path-list">${paths.map(path=>`<button type="button" class="critical-path-option ${path.critical_type||''} ${state.selectedCriticalPathId===path.path_id?'active':''}" onclick="selectCriticalPath('${esc(path.path_id)}')"><span>${esc(criticalTypeLabel(path.critical_type))}${path.resource_scope?` · ${path.resource_scope==='makespan'?'总工期':'交付影响'}`:''} · ${esc(path.path_id)} · 第 ${esc(path.rank)} 路</span><strong>${esc((path.process_ids||[]).join(' → '))}</strong><small>路径工时 ${esc(criticalMinutes(path.duration_minutes))} · 时差 ${esc(criticalMinutes(path.total_slack_minutes))}</small></button>`).join('')}</div>`:'<div class="critical-path-empty">该工序具有关键标识，但代表路径已达到输出上限。</div>';$('#criticalPathBody').innerHTML=`<div class="critical-detail-grid">${details.map(([label,value])=>`<span>${esc(label)}</span><strong>${esc(value)}</strong>`).join('')}</div><div class="critical-path-section-head"><div><h4>所属代表路径</h4><p>D 为交付路径，R 为资源实际路径；选择后会在已加载甘特图中联动高亮。</p></div>${state.selectedCriticalPathId?'<button type="button" class="button ghost small" onclick="clearCriticalPathSelection()">清除高亮</button>':''}</div>${pathHtml}`;$('#criticalPathOverlay').classList.remove('hidden')}
function closeCriticalPathInfo(){const overlay=$('#criticalPathOverlay');if(overlay)overlay.classList.add('hidden')}
function selectCriticalPath(pathId){state.selectedCriticalPathId=String(pathId||'');closeCriticalPathInfo();applyCriticalPathHighlight()}
function clearCriticalPathSelection(){state.selectedCriticalPathId='';applyCriticalPathHighlight();closeCriticalPathInfo()}
function applyCriticalPathHighlight(){const selected=state.selectedCriticalPathId;document.querySelectorAll('.gantt-bar[data-critical-paths]').forEach(bar=>{const ids=String(bar.dataset.criticalPaths||'').split(',').filter(Boolean),matched=!!selected&&ids.includes(selected);bar.classList.toggle('gantt-path-selected',matched);bar.classList.toggle('gantt-path-muted',!!selected&&!matched)})}
function safeClassToken(value){return String(value||'').replace(/[^a-zA-Z0-9_-]/g,'-')}
function ganttBar(item,scale,extraClass='',showTooltip=true,manualLockMenu=false){const start=new Date(item.plan_start_time).getTime(),end=new Date(item.plan_end_time).getTime(),left=(start-scale.min)/scale.span*100,width=Math.max((end-start)/scale.span*100,.7),tooltip=showTooltip&&item.process_id?' onmouseenter="showGanttTooltip(event,this)" onmousemove="moveGanttTooltip(event)" onmouseleave="hideGanttTooltip()"':'',menuEnabled=manualLockMenu&&(item.allow_manual_lock||item.allow_manual_adjustment),context=menuEnabled?` oncontextmenu="openProcessContextMenu(event,'${processLockPayload(item)}')"`:'',adjustable=manualLockMenu&&item.allow_manual_adjustment&&extraClass!=='gantt-process-bar',drag=adjustable?` draggable="true" ondragstart="startGanttAdjustmentDrag(event,'${processLockPayload(item)}')" ondragend="endGanttAdjustmentDrag()"`:'',pathIds=criticalPathIds(item),criticalClasses=`${item.delivery_critical?'gantt-critical-delivery':''} ${item.resource_critical?'gantt-critical-resource':''}`,pathClass=state.selectedCriticalPathId?(pathIds.includes(state.selectedCriticalPathId)?'gantt-path-selected':'gantt-path-muted'):'';return `<div class="gantt-bar ${extraClass} ${adjustable?'gantt-bar-adjustable':''} gantt-status-${safeClassToken(item.status)} ${item.manually_locked?'gantt-bar-locked':''} ${item.has_conflict?'gantt-bar-conflict':''} ${criticalClasses} ${pathClass}" style="left:${left}%;width:${width}%" data-process-id="${esc(item.process_id||'')}" data-critical-paths="${esc(pathIds.join(','))}"${tooltip}${context}${drag}><span>${esc(item.process_id||item.order_id)}</span>${criticalMarker(item)}${lockMarker(item)}</div>`}
function showGanttTooltip(event,element){const tooltip=$('#ganttTooltip'),item=state.criticalPathItems.get(String(element.dataset.processId||'')),fields=item?ganttTooltipFields(item):[];tooltip.innerHTML=fields.map(([label,value])=>`<div class="gantt-tooltip-row"><span>${esc(label)}</span><strong>${esc(value)}</strong></div>`).join('');tooltip.classList.remove('hidden');moveGanttTooltip(event)}
...
function orderCriticalSummary(items){const structural=[...new Set(items.flatMap(item=>item.structural_path_ids||[]))],delivery=[...new Set(items.flatMap(item=>item.delivery_path_ids||[]))],resource=[...new Set(items.flatMap(item=>item.resource_path_ids||[]))];return{structural,delivery,resource,html:(structural.length||delivery.length||resource.length)?` · <b class="order-critical-count"><i class="delivery">D${delivery.length}</i><i class="resource">R${resource.length}</i></b>`:''}}
function orderSummaryItem(orderId,items,critical){return{order_id:orderId,process_id:orderId,plan_start_time:items.reduce((value,item)=>new Date(item.plan_start_time)<new Date(value)?item.plan_start_time:value,items[0].plan_start_time),plan_end_time:items.reduce((value,item)=>new Date(item.plan_end_time)>new Date(value)?item.plan_end_time:value,items[0].plan_end_time),structural_path_ids:critical.structural,delivery_path_ids:critical.delivery,resource_path_ids:critical.resource,is_order_summary:true}}
function orderGroupStartTime(items){const values=items.map(item=>new Date(item.plan_start_time).getTime()).filter(Number.isFinite);return values.length?Math.min(...values):Number.POSITIVE_INFINITY}
function sortedOrderGroupEntries(grouped){return Object.entries(grouped).sort(([leftId,leftItems],[rightId,rightItems])=>{const leftStart=orderGroupStartTime(leftItems),rightStart=orderGroupStartTime(rightItems);return leftStart!==rightStart?leftStart-rightStart:String(leftId).localeCompare(String(rightId),'zh-CN')})}
function renderPlanOrderGantt(rows){const context=state.planVersionView,scale=ganttScale(rows);if(!context||!scale)return '<div class="empty">无可展示的排程工序</div>';const grouped={};scale.valid.forEach(item=>(grouped[item.order_id||'未分配订单']??=[]).push(item));context.orderGroups={};return `<div class="gantt-shell"><div class="gantt gantt-order"><div class="gantt-row gantt-axis"><div class="gantt-label">订单 / 工序</div><div class="gantt-track">${scale.axis}${scale.nowLine}</div></div>${sortedOrderGroupEntries(grouped).map(([orderId,items],index)=>{items.sort((a,b)=>(a.sequence??0)-(b.sequence??0)||new Date(a.plan_start_time)-new Date(b.plan_start_time));const groupId=`plan-order-gantt-${index}`,critical=orderCriticalSummary(items),summary=orderSummaryItem(orderId,items,critical);context.orderGroups[groupId]={items,scale,loaded:false};return `<div class="gantt-row gantt-order-toggle" onclick="togglePlanOrderGantt('${groupId}',this)"><div class="gantt-label" title="${esc(orderId)}"><span class="gantt-caret">▶</span>${esc(orderId)} <small>${items.length} 道${critical.html}</small></div><div class="gantt-track">${scale.nowLine}${ganttBar(summary,scale,'gantt-order-bar',false)}</div></div>`}).join('')}</div></div>`}
function togglePlanOrderGantt(groupId,row){const group=state.planVersionView?.orderGroups?.[groupId];if(!group)return;let children=[...document.querySelectorAll(`[data-order-group="${groupId}"]`)];if(!group.loaded){row.insertAdjacentHTML('afterend',group.items.map(item=>`<div class="gantt-row gantt-child" data-order-group="${groupId}"><div class="gantt-label" title="${esc(item.process_id)}">${esc(item.process_id)}${criticalLabelDots(item)}<small>${esc(item.process_name||'')}</small></div><div class="gantt-track">${group.scale.nowLine}${ganttBar(item,group.scale,'gantt-process-bar',true,true)}</div></div>`).join(''));group.loaded=true;children=[...document.querySelectorAll(`[data-order-group="${groupId}"]`)];applyCriticalPathHighlight()}else{const collapse=children.some(item=>!item.classList.contains('gantt-hidden'));children.forEach(item=>item.classList.toggle('gantt-hidden',collapse))}const expanded=children.some(item=>!item.classList.contains('gantt-hidden')),caret=row.querySelector('.gantt-caret');if(caret)caret.textContent=expanded?'▼':'▶'}
function renderOrderGantt(rows){const scale=ganttScale(rows);if(!scale)return '<div class="empty">无可展示的生效排程工序</div>';const grouped={};scale.valid.forEach(item=>(grouped[item.order_id||'未分配订单']??=[]).push(item));return `<div class="gantt-shell"><div class="gantt gantt-order"><div class="gantt-row gantt-axis"><div class="gantt-label">订单 / 工序</div><div class="gantt-track">${scale.axis}${scale.nowLine}</div></div>${sortedOrderGroupEntries(grouped).map(([orderId,items],index)=>{items.sort((a,b)=>(a.sequence??0)-(b.sequence??0)||new Date(a.plan_start_time)-new Date(b.plan_start_time));const groupId=`order-gantt-${index}`,critical=orderCriticalSummary(items),summary=orderSummaryItem(orderId,items,critical);return `<div class="gantt-row gantt-order-toggle" onclick="toggleOrderGantt('${groupId}',this)"><div class="gantt-label" title="${esc(orderId)}"><span class="gantt-caret">▼</span>${esc(orderId)} <small>${items.length} 道${critical.html}</small></div><div class="gantt-track">${scale.nowLine}${ganttBar(summary,scale,'gantt-order-bar',false)}</div></div>${items.map(item=>`<div class="gantt-row gantt-child" data-order-group="${groupId}"><div class="gantt-label" title="${esc(item.process_id)}">${esc(item.process_id)}${criticalLabelDots(item)}<small>${esc(item.process_name||'')}</small></div><div class="gantt-track">${scale.nowLine}${ganttBar(item,scale,'gantt-process-bar',true,true)}</div></div>`).join('')}`}).join('')}</div></div>`}
function criticalGanttLegend(){return '<div class="critical-gantt-legend"><span><i class="delivery">D</i><span><strong>交付关键</strong><small>基于订单交期与时差识别，红色 D 表示已经延期</small></span></span><span><i class="resource">R</i><span><strong>资源关键</strong><small>由设备、人员占用关系形成的实际关键路径</small></span></span></div>'}
function withCriticalGanttLegend(html){return String(html).includes('class="gantt-shell"')?criticalGanttLegend()+html:html}
const renderResourceGanttBase=renderResourceGantt,renderPlanOrderGanttBase=renderPlanOrderGantt,renderOrderGanttBase=renderOrderGantt;
renderResourceGantt=function(...args){return withCriticalGanttLegend(renderResourceGanttBase(...args))};
renderPlanOrderGantt=function(...args){return withCriticalGanttLegend(renderPlanOrderGanttBase(...args))};
renderOrderGantt=function(...args){return withCriticalGanttLegend(renderOrderGanttBase(...args))};
...
async function reviewVersion(id,decision){const comment=prompt(decision===APS.versionStatus.APPROVED?'请输入审批意见（可选）':'请输入驳回原因')??null;if(comment===null)return;try{await api(`/api/versions/${encodeURIComponent(id)}/review`,{method:'POST',body:JSON.stringify({decision,comment})});closeModal();toast(decision===APS.versionStatus.APPROVED?'审批完成':'版本已驳回');renderVersions()}catch(e){toast(e.message,true)}}
...
function defaultRecord(){const ids={order:'order_id',machine:'machine_id',worker:'worker_id',resource_group:'resource_group_id',calendar:'calendar_id'},record={[ids[state.masterType]]:'NEW_ID',status:'ACTIVE'};if(state.masterType==='worker')record.skills=[];return record}
...
function openSnapshotImport(){showModal('导入完整数据快照',`<form id="snapshotImportForm"><p class="muted">完整业务快照必须包含 machine_calendars、machine_profiles、worker_profiles、resource_group_profiles、order_processes 五个根字段；machine_calendars 必须包含机加、热表、装配日历各一份。单类数组数据请使用下方“批量导入”。</p><label class="button ghost" style="display:inline-block;margin-bottom:12px">选择 JSON 文件<input id="snapshotImportFile" type="file" accept="application/json" hidden></label><textarea id="snapshotImportText" class="json-editor" placeholder="粘贴完整业务主数据快照 JSON"></textarea><div class="form-actions" style="display:flex;margin-top:14px"><button type="button" class="button ghost" data-close-modal>取消</button><button class="button primary">导入完整快照</button></div></form>`);$('#snapshotImportFile').onchange=async e=>$('#snapshotImportText').value=await e.target.files[0].text();$('#snapshotImportForm').onsubmit=async e=>{e.preventDefault();try{const data=JSON.parse($('#snapshotImportText').value);if(!data||Array.isArray(data)||typeof data!=='object')throw new Error('完整快照必须是 JSON 对象，单类数组请使用批量导入');const required=['machine_calendars','machine_profiles','worker_profiles','resource_group_profiles','order_processes'];const missing=required.filter(key=>!(key in data));if(missing.length)throw new Error(`完整快照缺少字段：${missing.join(', ')}`);const calendarTypes=(data.machine_calendars||[]).map(item=>item.schedule_type).sort();if(JSON.stringify(calendarTypes)!==JSON.stringify(['assembly','heat_treatment','machining']))throw new Error('machine_calendars 必须且只能包含机加、热表、装配日历各一份');const r=await api('/api/master-data/import',{method:'POST',body:JSON.stringify(data)});closeModal();toast(`完整快照导入成功，共处理 ${r.imported} 条记录`);renderMaster()}catch(err){toast(err.message,true)}}}
...
function enhanceTaskModal(defaults){const form=$('#taskForm');if(!form)return;const topsis=form.querySelector('.task-config-section');topsis.insertAdjacentHTML('beforebegin',`<div class="full task-config-section ga-config-section"><div class="ga-config-head"><div><h4>遗传算法规模参数</h4><p>当前工艺有效工序数：<strong id="taskProcessCount">0</strong></p></div><span id="taskGaScaleHelp"></span></div><div class="ga-parameter-grid"><label>种群数量<input id="taskPopulationSize" type="number" min="1" step="1" required></label><label>进化代数<input id="taskGenerations" type="number" min="0" step="1" required></label></div><p class="field-help">系统根据工序规模自动给出默认值，提交前可由用户修改。种群设为 1、代数设为 0 时，仅生成初始排程方案，不执行遗传寻优。</p></div>`);const textarea=form.querySelector('[name="config_overrides"]');cleanTaskOverrideJson(textarea);applyGaTaskDefaults(form,defaults);const scheduleType=form.querySelector('[name="schedule_type"]');scheduleType.addEventListener('change',async()=>{if(!scheduleType.value)return;try{const next=await api(`/api/tasks/defaults?schedule_type=${encodeURIComponent(scheduleType.value)}`);applyGaTaskDefaults(form,next);const startInput=form.querySelector('[name="schedule_start"]');if(startInput&&next.schedule_start)startInput.value=String(next.schedule_start).slice(0,16)}catch(error){toast(error.message,true)}});const baseSubmit=form.onsubmit;form.onsubmit=event=>{const population=Number($('#taskPopulationSize').value),generations=Number($('#taskGenerations').value);if(!Number.isInteger(population)||population<1||!Number.isInteger(generations)||generations<0){event.preventDefault();toast('种群数量必须是大于等于 1 的整数，进化代数必须是大于等于 0 的整数',true);return}try{const config=JSON.parse(textarea.value||'{}');config.nsga3={...(config.nsga3||{}),population_size:population,generations};config.nsga3.hierarchical={...(config.nsga3.hierarchical||{}),population_size:population,generations};config.nsga3.hybrid_large_scale={...(config.nsga3.hybrid_large_scale||{}),local_population_size:population,local_generations:generations};textarea.value=JSON.stringify(config)}catch{}return baseSubmit.call(form,event)}}
const taskWeightFields=['weight_makespan','weight_tardiness_count','weight_total_tardiness','weight_machine_idle_rate','weight_machine_balance','weight_worker_balance','weight_wip_waiting'];
const taskWeightPresets=[
    {key:'balanced',label:'综合均衡',description:'兼顾交付、设备效率和生产等待',values:[0.15,0.20,0.15,0.20,0.15,0.05,0.10]},
    {key:'delivery',label:'交付优先',description:'重点减少延期订单数量和延期时长',values:[0.10,0.30,0.25,0.10,0.10,0.05,0.10]},
    {key:'equipment',label:'设备效率',description:'重点降低设备空闲并缩短整体完工时间',values:[0.20,0.10,0.10,0.30,0.15,0.05,0.10]},
    {key:'balance',label:'负荷均衡',description:'重点平衡设备和工人的任务负荷',values:[0.10,0.10,0.10,0.15,0.30,0.20,0.05]}
];
function matchingTaskWeightPreset(form){return taskWeightPresets.find(preset=>taskWeightFields.every((name,index)=>Math.abs((Number(form.elements[name]?.value)||0)-preset.values[index])<1e-9))||null}
function syncTaskWeightPreset(form){const matched=matchingTaskWeightPreset(form);form.querySelectorAll('[data-weight-preset]').forEach(button=>{const active=button.dataset.weightPreset===matched?.key;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active))});const stateLabel=form.querySelector('#taskWeightPresetState');if(stateLabel)stateLabel.textContent=matched?`当前：${matched.label}`:'当前：自定义'}
function applyTaskWeightPreset(form,presetKey){const preset=taskWeightPresets.find(item=>item.key===presetKey);if(!preset)return;preset.values.forEach((value,index)=>{form.elements[taskWeightFields[index]].value=value.toFixed(2)});const total=preset.values.reduce((sum,value)=>sum+value,0),totalElement=form.querySelector('#weightTotal');if(totalElement){totalElement.textContent=`权重合计：${total.toFixed(2)}`;totalElement.classList.toggle('invalid',Math.abs(total-1)>0.0001)}syncTaskWeightPreset(form)}
const enhanceTaskModalBase=enhanceTaskModal;
enhanceTaskModal=function(defaults){const result=enhanceTaskModalBase(defaults),form=$('#taskForm');if(!form)return result;const weightSection=[...form.querySelectorAll('.task-config-section')].find(section=>section.querySelector('[name="weight_makespan"]')),title=weightSection?.querySelector('h4');if(!weightSection||!title)return result;title.textContent='方案选择权重';const head=document.createElement('div');head.className='weight-section-head';title.before(head);head.appendChild(title);head.insertAdjacentHTML('beforeend',`<div class="weight-preset-controls"><div class="weight-preset-buttons" role="group" aria-label="常用方案选择权重">${taskWeightPresets.map(preset=>`<button type="button" class="weight-preset-button" data-weight-preset="${preset.key}" title="${esc(preset.description)}" aria-pressed="false">${esc(preset.label)}</button>`).join('')}</div><span id="taskWeightPresetState" class="weight-preset-state"></span></div>`);head.querySelectorAll('[data-weight-preset]').forEach(button=>button.onclick=()=>applyTaskWeightPreset(form,button.dataset.weightPreset));taskWeightFields.forEach(name=>form.elements[name].addEventListener('input',()=>syncTaskWeightPreset(form)));syncTaskWeightPreset(form);return result};
function enableCompactTaskModal(form){const modal=form.closest('.modal-card');modal?.classList.add('schedule-task-modal');form.querySelector('[name="config_overrides"]')?.closest('label')?.classList.add('task-json-field');form.querySelector('[name="local_adjustments"]')?.closest('label')?.classList.add('task-local-field');const mode=form.elements.mode,syncModeClass=()=>form.classList.toggle('local-mode',mode?.value==='local');mode?.addEventListener('change',syncModeClass);syncModeClass()}
const enhanceTaskModalLayoutBase=enhanceTaskModal;
enhanceTaskModal=function(defaults){const result=enhanceTaskModalLayoutBase(defaults),form=$('#taskForm');if(form)enableCompactTaskModal(form);return result};
function selectedPlanVersionIds(){const available=new Set(state.versions.map(version=>String(version.version_id)));return [...state.selectedVersionIds].filter(versionId=>available.has(String(versionId)))}
function toggleVersionSelection(versionId,checked){if(checked)state.selectedVersionIds.add(versionId);else state.selectedVersionIds.delete(versionId);syncVersionSelectionControls()}
function toggleVisibleVersionSelection(checked){filteredPlanVersions().forEach(version=>{if(checked)state.selectedVersionIds.add(version.version_id);else state.selectedVersionIds.delete(version.version_id)});syncVersionSelectionControls()}
function syncVersionSelectionControls(){const selected=selectedPlanVersionIds(),visible=filteredPlanVersions().map(version=>String(version.version_id)),selectedSet=new Set(selected.map(String));document.querySelectorAll('.version-select-checkbox').forEach(input=>{input.checked=selectedSet.has(String(input.dataset.versionId))});const selectAll=$('#versionSelectAll'),selectedVisible=visible.filter(versionId=>selectedSet.has(versionId)).length;if(selectAll){selectAll.checked=visible.length>0&&selectedVisible===visible.length;selectAll.indeterminate=selectedVisible>0&&selectedVisible<visible.length;selectAll.disabled=!visible.length}const button=$('#selectedVersionCompareButton');if(button){button.disabled=selected.length<2;button.textContent=`已选版本指标对比（${selected.length}）`}}
function versionTableEnhanced(rows){if(!rows.length)return '<div class="empty">暂无计划版本，请先执行排程任务</div>';return `<div class="table-wrap version-table-wrap"><table class="data-table version-list-table"><thead><tr><th><label class="version-select-control version-select-all"><input id="versionSelectAll" type="checkbox" onchange="toggleVisibleVersionSelection(this.checked)"><span>版本</span></label></th><th>工艺类型</th><th>排程模式 / 派工规则</th><th>来源任务</th><th>种群 / 代数</th><th>排程用时</th><th>状态</th><th>创建人</th><th>创建时间</th><th>审批 / 发布</th><th></th></tr></thead><tbody>${rows.map(v=>`<tr><td><label class="version-select-control"><input type="checkbox" class="version-select-checkbox" data-version-id="${esc(v.version_id)}" ${state.selectedVersionIds.has(v.version_id)?'checked':''} onchange="toggleVersionSelection(this.dataset.versionId,this.checked)"><span><b>${esc(v.version_id)}</b><span class="record-sub">#${v.version_no}</span></span></label></td><td>${scheduleTypeBadge(v.schedule_type)}</td><td>${scheduleModeBadge(v.mode)}<div class="record-sub">${esc(comparisonDispatchingLabel(v.dispatching_rule))}</div></td><td class="mono">${esc(v.task_id)}</td><td><b>${v.population_size??'未记录'} / ${v.generations??'未记录'}</b><div class="record-sub">种群 / 代</div></td><td>${esc(formatRunDuration(v.duration_seconds))}</td><td>${badge(v.status)}</td><td>${esc(v.created_by||'-')}</td><td>${esc(String(v.created_at||'-').replace('T',' '))}</td><td><div class="record-sub">${esc(v.reviewed_by||'-')} / ${esc(v.published_by||'-')}</div></td><td><button class="button ghost small" onclick="showVersion('${esc(v.version_id)}')">打开计划</button></td></tr>`).join('')}</tbody></table></div>`}
...
function renderMultiVersionMetricComparison(data){const versions=data.versions||[],definitions=data.metric_definitions||[];return `<div class="multi-compare-heading"><div><h3>多版本核心指标对比</h3><p>仅展示汇总指标；绿色单元格为当前指标最优版本</p></div><span>${versions.length} 个版本</span></div><div class="multi-compare-table-wrap"><table class="multi-compare-table"><thead><tr><th>核心指标</th>${versions.map(version=>`<th><strong>${esc(version.version_id)}</strong><small>#${esc(version.version_no??'-')} · ${esc(scheduleTypeLabel(version.schedule_type))} · ${esc(version.status||'-')}</small></th>`).join('')}</tr></thead><tbody>${definitions.map(metric=>{const bestIds=new Set(metric.best_version_ids||[]);return `<tr><th><strong>${esc(metric.label)}</strong><small>${metric.better_when==='higher'?'数值越高越优':'数值越低越优'}</small></th>${versions.map(version=>{const best=bestIds.has(version.version_id);return `<td class="${best?'multi-compare-best':''}"><strong>${esc(comparisonMetricValue(version.metrics?.[metric.key],metric.unit))}</strong>${best?'<span>最优</span>':''}</td>`}).join('')}</tr>`}).join('')}</tbody></table></div>`}
async function openSelectedVersionComparison(){const versionIds=selectedPlanVersionIds();if(versionIds.length<2){toast('请至少勾选两个计划版本',true);return}showModal('已选版本指标对比','<div class="loading">正在加载核心指标…</div>');document.querySelector('.modal-card').classList.add('compare-modal','multi-compare-modal');try{const data=await api('/api/versions/compare-metrics',{method:'POST',body:JSON.stringify({version_ids:versionIds})});$('#modalBody').innerHTML=renderMultiVersionMetricComparison(data)}catch(error){$('#modalBody').innerHTML=`<div class="empty">${esc(error.message)}</div>`;toast(error.message,true)}}
function comparisonModeLabel(value){return(APS.scheduleModeLabels[value]||value||'未记录')}
...
const orderMasterTypes=[
    {value:APS.scheduleType.MACHINING,label:'机加订单'},
    {value:APS.scheduleType.HEAT_TREATMENT,label:'热表订单'},
    {value:APS.scheduleType.ASSEMBLY,label:'装配订单'}
];
const calendarMasterTypes=[
    {value:APS.scheduleType.MACHINING,label:'机加日历'},
    {value:APS.scheduleType.HEAT_TREATMENT,label:'热表日历'},
    {value:APS.scheduleType.ASSEMBLY,label:'装配日历'}
];
const orderBusinessTypeMapping=APS.orderBusinessTypeMapping;
const masterOrderBusinessTypes=APS.scheduleOrderBusinessTypes;
state.masterOrderType=state.masterOrderType||APS.scheduleType.MACHINING;
...
function ensureOrderScope(record,scheduleType){
    const allowed=masterOrderBusinessTypes[scheduleType]||[];
    let businessType=String(record?.order_business_type||'').toUpperCase();
    if(!businessType)businessType=allowed[0]||'';
    if(!allowed.includes(businessType))throw new Error('订单业务类型与当前“'+orderMasterTypeLabel(scheduleType)+'”不一致');
    const result={...record,order_business_type:businessType};
...
function currentMasterScopeLabel(){
    if(state.masterType==='order')return orderMasterTypeLabel(state.masterOrderType);
    if(state.masterType==='calendar')return calendarMasterTypeLabel(state.masterCalendarType);
    return entityLabels[state.masterType]||state.masterType;
}

function currentMasterDeleteUrl(){
    const base='/api/master-data/'+state.masterType;
    if(state.masterType==='order')return base+'?schedule_type='+encodeURIComponent(state.masterOrderType);
    if(state.masterType==='calendar')return base+'?schedule_type='+encodeURIComponent(state.masterCalendarType);
    return base;
}

async function deleteAllMaster(){
    const count=visibleMasterRecords().length;
    const label=currentMasterScopeLabel();
    if(!count){
        toast(label+'暂无可删除数据',true);
        return;
    }
    if(!confirm('确认一键删除当前'+label+'的全部 '+count+' 条数据？\n此操作不可恢复，历史任务快照不受影响。'))return;
    try{
        const result=await api(currentMasterDeleteUrl(),{method:'DELETE'});
        toast('已删除 '+result.deleted+' 条'+label+'数据');
        renderMaster();
    }catch(error){
...
function orderMasterTabsHtml(){
    ...
    const fixed=deploymentScheduleTypes[state.systemConfig?.deployment_process_type];
    const options=fixed?orderMasterTypes.filter(option=>option.value===fixed):orderMasterTypes;
    return '<div class="order-type-tabs">'+options.map(option=>{
...
async function renderMasterByOrderType(){
    try{
        ...
        const deleteAction='<button class="button danger" onclick="deleteAllMaster()"'+(visible.length?'':' disabled')+'>一键删除'+esc(currentMasterScopeLabel())+'</button>';
        const recordActions='<div class="toolbar master-toolbar"><input id="masterSearch" class="search" placeholder="搜索编号或名称…"><div class="toolbar-group">'+batchActions+deleteAction+newRecordAction+'</div></div>';
...
function switchOrderType(scheduleType){
    ...
    const fixed=deploymentScheduleTypes[state.systemConfig?.deployment_process_type];
    if(fixed&&scheduleType!==fixed)return;
...
function defaultCalendarWeeklyShifts(scheduleType){
    if(scheduleType===APS.scheduleType.HEAT_TREATMENT){
...
function defaultRecordByOrderType(){
    ...
    return {
        ...
        order_business_type:(masterOrderBusinessTypes[state.masterOrderType]||[])[0]||'',
        order_type:'STANDARD',
        product_id:'',
        product_name:'',
        material:'',
...
async function renderVersionsWithSearch(){
    try{
        ...
        const availableVersionIds=new Set(state.versions.map(version=>String(version.version_id)));
        state.selectedVersionIds=new Set([...state.selectedVersionIds].filter(versionId=>availableVersionIds.has(String(versionId))));
        const filters=state.versionFilters||{};
        ...
        content.innerHTML='<section class="panel">'
            +'<div class="panel-header"><div><h3>排程计划版本</h3><p>算法结果需完成业务审批后才能发布并回写工序</p></div><div class="version-compare-actions"><button id="selectedVersionCompareButton" class="button secondary" type="button" onclick="openSelectedVersionComparison()" disabled>已选版本指标对比（0）</button><button class="button ghost" type="button" onclick="openCompare()">版本对比</button></div></div>'
...
async function renderTasksWithSearch(){
    try{
        ...
        updateTaskSearchResults();
        if(state.tasks.some(task=>APS.activeTaskStatuses.includes(task.status))){

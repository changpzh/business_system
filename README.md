...
- [业务单元部署与虚拟资源使用说明](docs/业务单元部署与虚拟资源使用说明.md)
...
- 按 `MACHINING / HEAT_TREATMENT / ASSEMBLY / DEBUG` 配置部署业务单元，动态显示系统标题并强制任务工艺范围；
- 订单通过必填 `order_business_type` 管理业务归属，任务选中订单后保留完整工艺路线；
- 真实/虚拟资源统一维护，虚拟工序支持无限并行、时间锁和外部确认窗口；
- 机加、热表、装配三类日历独立维护，不使用名称推断或旧日历自动迁移；
...
│   ├── domain.py            # 订单归属、任务工艺、部署类型等领域规则
│   ├── system_configuration.py # 业务单元配置、锁定保护和审计
...
1. 管理员先在“业务单元配置”确定正式业务单元；调试环境可保持 `DEBUG`。
2. 在“主数据中心”维护或导入同一业务版本的订单、真实/虚拟资源和三类日历。
3. 在“排程任务”核对正式部署固定工艺，或在调试部署选择工艺，再设置模式、派工规则和算法参数。
4. 业务系统按订单归属固化任务快照，保留所选订单全部工序，生成唯一 `task_id` 并调用算法服务。
5. 成功结果自动形成 `DRAFT` 计划版本；失败任务可保留原 `task_id` 重试。
6. 审批人查看 KPI、甘特图和明细，执行通过或驳回。
7. `APPROVED` 版本发布后成为当前生产计划，旧发布版本转为 `SUPERSEDED`。
8. 发布结果回写工序计划时间、设备、人员和版本号；人工锁继续独立维护。

## API 概览

- `POST /api/auth/login`：登录；
- `GET /api/dashboard`：运营总览；
- `GET/PUT /api/system-configuration`：业务单元部署配置；
